  ...
  run: async (client, interaction) => {
    try {
      ...
      if (args && args.length > 0) {
        ...
        //     const embed = new EmbedBuilder()
        //        .setAuthor({ name: `Emote Manager Help`, iconURL: `${client.user.displayAvatarURL()}` })
        ...
        //     {
        //     name: 'Emotes',
        //      value: '`add` | `addmultiple` | `emojiinfo` | `enlarge` | `remove` | `rename`',
        //      inline: false
        //  }
        // ])
        //   .addFields([
        //     {
        //     name: 'General',
        //      value: '`help` | `invite` | `support` | `vote`',
        //      inline: false
        //  }
        // ])
        // .addFields([
        ...
        //     {
        //     name: 'Utilities',
        //      value: '`botinfo` | `ping` | `shard` | `stats` | `uptime`',
        //      inline: false
        //  }
        // ])
        //       .setColor(ee.color)
        ...
        const embed = new EmbedBuilder()
          .setColor(ee.color)
          .setAuthor({
            name: `Emosticorn Help`,
            iconURL: client.user.displayAvatarURL(),
          })
          .setThumbnail(client.user.displayAvatarURL())
          .setDescription(
            `• Prefix for this server is \`/\` \n • Type </general help:1099354394413236315> \`<command>\` for more info`
          )
        ...
        try {
          ...
          for (const category of client.categories) {
            ...
            const final_list = client.helpMaker
              .filter((obj) => obj.name === category.toLowerCase())
              .map((cmd) =>
                items
                  .map((item) => `</${cmd.name} ${item}:${cmd.id}>`)
                  .join(" | ")
              );

            embed.addFields({
              name: `**${category.toUpperCase()} [${items.length}]**`,
              value: `> ${final_list.join(" | ")}`,
            });
          }
        } catch (e) {
          console.log(String(e.stack).red);
